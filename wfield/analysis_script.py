...
x, rPSTHs,ste = psth_from_df(U,SVTcorr,behavior,sync_behavior,atlas, 3, ccf_regions,localdisk,ax=ax, events='feedback_times', split_on='feedbackType',window=[-20,60],smoothing=None)
show()
error_correct_mov(rPSTHs,rPSTHs, nref_regions, fname='correct_vs_error.gif')
...
def error_correct_mov(rPSTHs,lPSTHs, nref_regions, fname=None):
    ...
    for ii in np.arange(len(rPSTHs)):
        ...
        if ii == 0:
            plt.text(xoff+rPSTHs[ii].shape[2]*0.8,50,'Incorrect',ha='center',va = 'bottom')  
        else:
            plt.text(xoff+rPSTHs[ii].shape[2]*0.8,50,'Correct',ha='center',va = 'bottom')   
        # plt.text(xoff+rPSTHs[ii].shape[2]*0.8,50,'{0:1.1f}'.format(prob_left[ii]),ha='center',va = 'top')
