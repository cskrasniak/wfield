...
subject = 'CSK-im-002'
date = '2021-01-16'
localdisk = r'F:\imaging_data\CSK-im-002\2021-01-16'
...
strong_r_frames = time_to_frames(sync_behavior,np.array(strong_r))
lick100 = time_to_frames(sync_behavior,np.array(behavior['response_times'].iloc[:100]))

strong_l = behavior[behavior['signedContrast']<-.4]['stimOn_times']
strong_l_frames = time_to_frames(sync_behavior,np.array(strong_l))
# Use unique to exclude the default value of frames that is assigned for nan values
first_move_l = np.unique(time_to_frames(sync_behavior, np.array(behavior[behavior['choice']==-1]['firstMovement_times'])))
first_move_r = np.unique(time_to_frames(sync_behavior, np.array(behavior[behavior['choice']==1]['firstMovement_times'])))
l_block_stim = np.unique(time_to_frames(sync_behavior, np.array(behavior[behavior['probabilityLeft'] >.5]['stimOn_times'])))
r_block_stim = np.unique(time_to_frames(sync_behavior, np.array(behavior[behavior['probabilityLeft'] <.5]['stimOn_times'])))
...
def time_to_frames(sync,event_times,fr=30):
    """
    ...
    """
    all_frames = np.arange(sync['frame'].iloc[0],sync['frame'].iloc[-1])
    frame_times = np.linspace(sync['task_time'].iloc[0],sync['task_time'].iloc[-1],
                            sync['frame'].iloc[-1] - sync['frame'].iloc[0])
    if np.isnan(sync.iloc[-1]['task_time']):
        frame_times = np.linspace(sync['task_time'].iloc[0],sync['task_time'].iloc[-2]+np.diff(sync.timestamp[-2:])[0],
                            sync['frame'].iloc[-1] - sync['frame'].iloc[0])
    ...
    for i in range(len(event_times)):
        event_frames[i] = all_frames[find_nearest(frame_times,event_times[i])]
    return event_frames
...
def frame_average_by_session(frames):
    """
    gives an average frame across the whole session to use to calculate df/f
    Input:
        frames: numpy array with nframes x nchannels x Wpixels x Hpixels
    Output:
        numpy array with nchannels x wPixels x Hpixels
    """
    assert frames.shape[1:] == (2,540,640), 'input is the wrong shape, should be (nframes,2,540,640)'
    return frames.mean(axis=0)

def df_over_f(frames,frames_average):
    """
    Simply calculates df/f by (f(t) - f(0))/f(0)
    """
    assert frames.shape[1:]==frames_average.shape[1:], 'the inputs must have the same shape after nframes'
    
    return (frames-frames_average)/frames_average

def peth_from_pixels(event_frames, U, SVT, indexes, window=[-30,120], smoothing=3):
    """"
    This creates a peth trace averaged for all the pixels from a given input. This is used 
    in peth_by_allen_area function and can be used other times when a peth averaged across 
    a spatial extent is needed.
    
    Warning! this function is agnostic to channel, so only use data input from a single channel
    
    Inputs:
        event_frames: a numpy array of the frames on which events occurred
        data: a 1D array of length num frames where each element is a df/f or intensity
        window: the window in number of frames around an event for which to make the peth 
        smoothing: sigma of the kernal in number of frames to apply 1d gaussian smoothing, if
                   None, then no smoothing occurs
    Outputs:
        the peth as a 1D numpy array the length of the window
    """
    from scipy.ndimage import gaussian_filter1d
    data_len = SVT.shape[1]
    if max(event_frames)/data_len>1.9:
        print('looks like this is data from one channel, downsampling the frames to match')
        event_frames = np.asarray(event_frames/2)
    slices = np.empty((len(event_frames),abs(window[0])+abs(window[1])))
    cnt=0
    for event in event_frames:
        ...
        if end > data_len:
            ...
            pad = np.empty([200,end-data_len])
            pad[:,:] = np.nan
            SVT = np.append(SVT,pad)
        slices[cnt,:] = reconstruct(U[indexes],SVT[start:end],dims=indexes.size)
        cnt+=1
    peth = slices.mean(axis=0)
    if smoothing:
        peth = gaussian_filter1d(peth,smoothing)
    x = np.arange(window[0],window[1])
    error = slices.std(axis=0)/np.sqrt(slices.shape[0])
    return x, peth,error
...
def peth_by_allen_area(event_frames, U,SVT, allen_idx, allen_atlas, window=[-10,60], smoothing=1,channel=0):
    ...
    # assert isinstance(allen_area,pd.Series) or isinstance(allen_area,pd.DataFrame), 'input allen_area must be a pandas series as returned by allen_load_reference'
    from scipy.ndimage import gaussian_filter1d
    # if hemisphere == 'both':
    #     allen_idx = [allen_area.label,-1*allen_area.label]
    # elif hemisphere == 'right':
    #     allen_idx = -1*allen_area.label
    # elif hemisphere == 'left':
    #     allen_idx = allen_area.label
    # use_pixels = U[(allen_atlas==allen_area.label)[0],(allen_atlas==allen_area.label)[1],:] #get the pixels corresponding to that area
   
    # if len(U.shape) == 3 and channel==0:
    #     use_pixels = U[:,::2,allen_atlas==allen_area.label]  
    # elif len(U.shape) == 3 and channel==1:
    #     use_pixels = U[:,1::2,allen_atlas==allen_area.label]
    # assert use_pixels.shape==2
    if type(allen_idx)==np.int64:
    ...
    # peth, x, error = peth_from_pixels(event_frames,area_mean_frames[:,1],window=window, smoothing=smoothing)
    if smoothing:
        mean_peth = gaussian_filter1d(mean_peth,smoothing)
    return x, mean_peth, mean_std

def whole_brain_peth(event_frames, U,SVT, window=[-2,20],smoothing=[1,1,1]):
    """
    ...
    # slices = np.empty((2,abs(window[0])+abs(window[1]), U.shape[0], U.shape[1]))
    cnt=1
    runsum = 0
    runsq = 0
    for event in tqdm(event_frames):
        ...
        if cnt == 1: #on first loop the first mean is just the first sample
            m = reconstruction
        ...
        cnt+=1
    std = np.sqrt(runsq/cnt - np.square(runsum/cnt))
    # peth = slices.mean(axis=0)
    if smoothing:
...
def plot_signal_summary(U, SVT, behavior, sync_behavior, allen_area, allen_atlas,events):
    ...
    x,peth_l = peth_by_allen_area(events,U,SVT,allen_area.label, allen_atlas,window=[-10,60])
    x,peth_r = peth_by_allen_area(events,U,SVT,allen_area.label*-1, allen_atlas,window=[-10,60])
    ax3.plot(x,peth_l,'b')
    ax3.plot(x,peth_r,'r')
    ax3.set_xlim(np.min(x),np.max(x))
    ax3.set_xticks([0,15,30,45,60])
    ax3.set_xticklabels([0,.5,1,1.5,2])
    ax3.axvline(0,0,1,color='k')
...
def plot_window_lag(corr_im, area1_label, area2_label,data_range=60):
    ...
    # ax2.set_ylabel("mean Pearson's R")
    ax2.text(.5,.8,'skew = {}, p = {}'.format(np.round(skewness,2),np.round(p,2)))
